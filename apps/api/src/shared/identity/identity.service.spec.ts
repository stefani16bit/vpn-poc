...
describe('IdentityService', () => {
	...
	describe('refreshSession', () => {
		...
		it('rotates a live token into a new one in the same family', async () => {
			...
			const outcome = await service.refreshSession('token-1', EXECUTOR as never);
		...
		it('marks the presented token spent', async () => {
			sessions['lockByTokenHash']?.mockResolvedValue(candidate());
			await service.refreshSession('token-1', EXECUTOR as never);
			expect(sessions['spendToken']).toHaveBeenCalled();
		});

		it('reports a replayed token as reuse, not a plain rejection', async () => {
			sessions['lockByTokenHash']?.mockResolvedValue(candidate({ spentAt: NOW }));
			await expect(service.refreshSession('token-1', EXECUTOR as never)).resolves.toEqual({
				kind: 'reuse_detected',
				sessionId: 'family-1',
			});
		});
		...
		it('rejects a token it never issued', async () => {
			...
			await expect(service.refreshSession('nope', EXECUTOR as never)).resolves.toEqual({
				kind: 'rejected',
			});
		});

		it('rejects when the family is already revoked', async () => {
			sessions['lockByTokenHash']?.mockResolvedValue(candidate({ revokedAt: NOW }));
			await expect(service.refreshSession('token-1', EXECUTOR as never)).resolves.toEqual({
				kind: 'rejected',
			});
		});
		...
		it('rejects an expired token', async () => {
			...
			sessions['lockByTokenHash']?.mockResolvedValue(candidate({ expiresAt }));
			await expect(service.refreshSession('token-1', EXECUTOR as never)).resolves.toEqual({
				kind: 'rejected',
			});
		});
